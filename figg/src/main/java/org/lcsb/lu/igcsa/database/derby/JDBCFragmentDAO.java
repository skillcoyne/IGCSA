package org.lcsb.lu.igcsa.database.derby;
...
/**
 * org.lcsb.lu.igcsa.database.derby
...
//        "WHERE vpb.chr = ? AND vpb.bin_id = ? and v.name = ? LIMIT ?, 1";
    ...
    String sql = "SELECT vpb.chr, vpb.bin_id, vpb.var_count, v.name FROM ( " +
        " SELECT ROW_NUMBER() OVER() AS rownum, " + tableName +".* " +
        " FROM " + tableName +
        " ) AS vpb INNER JOIN variation as v on v.id = vpb.variation_id " +
        " WHERE vpb.chr = ? AND vpb.bin_id = ? and v.name = ? and " +
        " rownum >= " + fragmentNumber + " and rownum < " + fragmentNumber+1;

    log.debug("getVariationCount(" + chromosome + ", " + gcBinId + ", " + variationName + ", " + fragmentNumber + "): " + sql);
    return (Integer) jdbcTemplate.query(sql, new Object[]{chromosome,  gcBinId, variationName}, new ResultSetExtractor<Object>()
    {
