...
sizes = data.frame()
colnames(sizes) = variation_types
...
size = data.frame()
del = table(d[d$var.type == 'deletion', 'var.length'])
size['0-10', 'deletion'] = sum( del[ as.character(c(0:10)) ], na.rm=T )
for (i in seq(100, 900, 100) )
  {
  print(i)
  min=i;max=i+100;
  size[ paste(min,max,sep="-"), 'deletion' ] = sum( del[ as.character(c(min:max)) ], na.rm=T)
  }
